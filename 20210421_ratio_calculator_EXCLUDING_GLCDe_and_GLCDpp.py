...
outputname = "results_CAFBA_for_all_M9[sin_glc]_con_caca_MEDIA_DE_RATIOS_con_caca_DE_ENTERO" #edited by hand later to include everything
...
def ratio_calculator(models, s_models, medium, media_db, outputdir=".",outputname=outputname, include_original=False, to_remove=None, reverse=False):
    ...
    for f,s in pairs:  
        ...
        ## NUEVO: modifico este modelo para que no tenga esas reacciones !!
        try:
            del(model.reactions["R_GLCDe"])
            print("glcde was deleted")
        except:
            print("this model doesnt have glcde")
        try:
            del(model.reactions["R_GLCDpp"])
            print("glcdpp was deleted")
        except:
            print("this model doesnt have glcdpp")
        ## y ya resuelvo:
        solution = CAFBA(model,   objective="Growth")    
        ...
        if ratio_of_pair>0: # if the second one does grow in the spent medium of the first one
            n+=1
            total.append(ratio_of_pair)
            
        results.append([medium,f.split("/")[-1],s.split("/")[-1],ratio_of_pair])
    try:
        media = sum(total)/n
    ...
    all_data=pd.DataFrame(results,columns=["Medio","Modelo","spent_model","Ratio"])
