...
def ratio_calculator(models, s_models, medium, media_db, outputdir=".",outputname="ratiocalc_result", include_original=False, to_remove=None, reverse=False):
    """
    ...
    fn=outputdir+"/"+outputname

    if os.path.isfile(fn): # if exists:
        exists=True
        try: 
            already_done=pd.read_csv(fn)
        except:
            print("failed")
    else: # if doesnt exist yet
        exists=False
        header=pd.DataFrame([["Medio","Modelo","spent_model","Ratio"]])
        header.to_csv(fn,index=False,header=False)
...
def apply(pair, medium, media_db, outputdir=".", outputname="ratiocalc_result", 
...
def worker(arg, q):    
    try:
        res=apply(pair=pair, medium=medium, media_db=media_db, outputdir=wd, include_original=True, 
...
def main():
    ...
    outputname = "2_E2_results_CAFBA_for_all_M9[sin_glc]_con_caca_MEDIA_DE_RATIOS_con_caca_DE_ENTERO" #edited by hand later to include everything
    ...
    pairs=ratio_calculator(models1,models2, medium, media_db, 
                           outputdir=wd, outputname=outputname,
                           include_original=True, to_remove=to_remove, 
                           reverse=reverse)


    #fire off workers
    jobs = []
    for pair in pairs[1:3]:# FIXX
        print("Now running pair :",pair)
        job = pool.apply_async(worker, (pair, q))
        jobs.append(job)
